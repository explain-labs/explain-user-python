"""BloodTimeVaryingElastance — port of
src/explain/component_models/BloodTimeVaryingElastance.js

Extends TimeVaryingElastance the same way BloodCapacitance extends Capacitance:
adds blood-composition tracking (to2, tco2, solutes, drugs, temperature,
viscosity) and overrides `volume_in` to mix the inflowing composition into the
current compartment using the classical one-way mixing formula:
...
Like BloodCapacitance, the mixing is one-way — the source compartment's
composition is NOT updated; `volume_out` does not touch composition.
"""
...
class BloodTimeVaryingElastance(TimeVaryingElastance):
    ...
    def volume_in(self, dvol: float, comp_from=None):
        ...
        # Guard against divide-by-zero. Matches the BloodCapacitance port's
        # behavior: JS produces NaN when this.vol == 0 after super — we skip
        # instead so a research run isn't polluted by NaN state. If you need
        # strict JS parity including NaN propagation, remove this guard.
